...
export class ExecutionContainer<T> {
  ...
  #retrieveFromContext = async (): Promise<
    { params: any; headers: any; query: any; body: any }
  > => {
  ...
  #executeBeforeFns = async (): Promise<void> => {
    ...
    const beforeFns: Array<Function> =
      this.#controllerMeta.beforeFns.get(methodName) ?? [];
  ...
  #getStatus = (): Status => {
    const isPostRequest: boolean =
      this.#route.requestMethod === HttpMethod.POST;
    return this.#controllerMeta.defaultResponseCodes.get(
      this.#route.methodName,
    ) ??
  ...
  async execute(): Promise<ExecutionResult> {
    ...
    try {
      const instance: any = new this.#controllerDefinition();
      const action: Function = instance[this.#route.methodName as string];

      // execute action here.
      const controllerResponse: any = await action(...args);
